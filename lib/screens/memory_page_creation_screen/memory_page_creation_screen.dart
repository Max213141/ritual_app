          ...
          if (state is Success) {
            showDialog(
              ...
              builder: (context) => SuccessDialogWidget(
                title: 'Успех',
                subtitle:
                    'Доска памяти создана, теперь вы можете получить QR, что бы поделиться ею',
                onPressed: () {
                  GoRouter.of(context).go('/home');

                  context.read<MemoryDeskBloc>().add(
                      const GetMemoryDesks()); // pop screen and send "refresh" signal
                },
              ),
            );
          }
        },
        builder: (coontext, state) => Stack(
          children: [
            SizedBox(
              height: screenHeight,
              child: AspectRatio(
